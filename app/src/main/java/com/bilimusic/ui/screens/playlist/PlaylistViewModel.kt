    ...
    fun playPlaylist(playlistId: String, startIndex: Int = 0) {
        viewModelScope.launch {
            try {
                ...
                // 网易云歌曲：获取音源
                if (target.url == null && target.source == "NETEASE" && target.neteaseId > 0) {
                    try {
                        val resp = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
                            com.bilimusic.data.api.netease.NeteaseApiClient.getSongUrl(target.neteaseId)
                        }
                        ...
                        if (url != null && url != "__VIP_ONLY__") {
                            ...
                            // VIP only or no URL - fallback to B站
                            val mutableSongs = songs.toMutableList()
                            fallbackNeteaseSong(target, mutableSongs, idx)
                            songs = repository.getSongsInPlaylistOnce(playlistId)
                        }
                    } catch (_: Exception) {
                        val mutableSongs = songs.toMutableList()
                        fallbackNeteaseSong(target, mutableSongs, idx)
                ...
                if (songs.getOrNull(idx)?.url != null || songs.getOrNull(idx)?.localPath != null) {

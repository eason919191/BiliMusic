package com.bilimusic.ui.screens.downloads
        ...
        val job = downloadScope.launch {
            try {
                ...
                if (task.musicId.startsWith("BV") || task.musicId.length >= 10) {
                    try {
                        ...
                        if (detail != null && detail.cid > 0) {
                            val audioOnly = com.bilimusic.data.api.BilibiliApiClient.getDownloadAudioUrl(bvid, detail.cid)
                            if (audioOnly != null) {
                ...
                val downloadDir = if (downloadPath.isNotBlank()) {
                    ...
                    File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MUSIC), "BiliMusic").apply { mkdirs() }
                }

...
object BilibiliApiClient {
    ...
    private val okHttpClient: OkHttpClient by lazy {
        ...
        OkHttpClient.Builder()
            ...
            .retryOnConnectionFailure(true)
                ...
                if (request.header("Cookie") == null && userCookie.isNotBlank()) {
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                while (hasMore) {
                    ...
                    Log.d(TAG, "Fav page=$page mediaId=$mediaId code=${response.code} msg=${response.message}")
            Log.d(TAG, "  pageInfo=$pageInfo medias.size=${medias?.size}")
                    if (response.code == 0 && medias != null) {
                        ...
                        Log.d(TAG, "  page=$page got ${videos.size} raw=${medias.size} accumulated=${allVideos.size} totalFromAPI=${pageInfo?.total}")
                        ...
                        Log.w(TAG, "Fav resource stopped: code=${response.code} msg=${response.message} at page=$page")
                        hasMore = false
                    }
                }
            ...
            try {
                ...
                if (subtitleResp.code == 0) {
                    ...
                    if (subs != null && subs.isNotEmpty()) {
                        ...
                        Log.d(TAG, "getVideoSubtitles: no subtitles in response, hasData=${subtitleResp.data != null}")
                        emptyList()
                    }
                } else {
                    Log.d(TAG, "getVideoSubtitles: code=${subtitleResp.code}, msg=${subtitleResp.message}")
                    emptyList()
                }
            } catch (e: Exception) {
     ...
     * 哔哩哔哩字幕JSON格式: [{from: float, to: float, content: string}, ...]
     */
    suspend fun fetchSubtitleContent(subtitleUrl: String): List<LyricLine> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                Log.d(TAG, "fetchSubtitleContent: url=$fullUrl")
                val reqBuilder = Request.Builder().url(fullUrl).get()
                if (userCookie.isNotBlank()) reqBuilder.addHeader("Cookie", userCookie)
                val response = okHttpClient.newCall(reqBuilder.build()).execute()
                val body = response.body?.string() ?: return@withContext emptyList()
                Log.d(TAG, "fetchSubtitleContent: body length=${body.length}, starts=${body.take(200)}")
                // AI字幕返回 {"body": [...]}，普通字幕返回 [...]
                val jsonArray = try {
    ...
    /**
     * 获取B站播放历史 - 使用 navigation/list API
     */
    suspend fun getPlayHistory(cookie: String, count: Int = 20): List<BilibiliVideo> {
        return withContext(Dispatchers.IO) {
            try {
                if (userCookie.isNotBlank() && !userCookie.contains("buvid3")) {
                    val buvid3 = java.util.UUID.randomUUID().toString().uppercase() +
                        (0 until 5).map { (Math.random() * 10).toInt() }.joinToString("") + "infoc"
                    userCookie = "$userCookie; buvid3=$buvid3"
                }

                val allResults = mutableListOf<BilibiliVideo>()
                var maxOid = 0L
                var viewAt = 0
                val pageSize = 20

                while (allResults.size < count) {
                    val url = "https://api.bilibili.com/x/web-interface/history/cursor?type=all&ps=$pageSize&max=$maxOid&view_at=$viewAt"
                    val reqBuilder = Request.Builder().url(url).get()
                    val response = okHttpClient.newCall(reqBuilder.build()).execute()
                    ...
                    if (code != 0) {
                        Log.e(TAG, "getPlayHistory API error: code=$code, message=${json.optString("message", "")}")
                        break
                    }
                    val data = json.optJSONObject("data") ?: break
                    val items = data.optJSONArray("list") ?: break
                    ...
                    for (i in 0 until items.length()) {
                        ...
                        val title = item.optString("title", "").replace(Regex("<[^>]*>"), "")
                        val author = item.optString("author_name", "")
                        val cover = item.optString("cover", "")
                        val duration = item.optLong("duration", 0L)
                        allResults.add(BilibiliVideo(
                    ...
                    val lastHistory = lastItem.optJSONObject("history")
                    maxOid = lastHistory?.optLong("oid", 0) ?: 0
                    viewAt = lastItem.optInt("view_at", 0)
                    if (maxOid == 0L) break
                    Log.d(TAG, "getPlayHistory: got ${items.length()} items, total ${allResults.size}, next max=$maxOid view_at=$viewAt")
                }
                Log.d(TAG, "getPlayHistory: total ${allResults.size} items")
                allResults.reversed()
            } catch (e: Exception) {
                Log.e(TAG, "getPlayHistory error", e)
                emptyList()
            }
        }
    }

    private fun genAuroraEid(uid: String): String {
        if (uid.isEmpty() || uid == "0") return ""
        val key = "ad1va46a7lza"
        val midBytes = uid.toByteArray(Charsets.US_ASCII)
        for (i in midBytes.indices) {
            midBytes[i] = (midBytes[i].toInt() xor key[i % key.length].code).toByte()
        }
        return android.util.Base64.encodeToString(midBytes, android.util.Base64.NO_WRAP).replace("=", "")
    }

    /**
     * 添加视频到哔哩哔哩收藏夹（先通过bvid获取avid，再用avid调API）
     */

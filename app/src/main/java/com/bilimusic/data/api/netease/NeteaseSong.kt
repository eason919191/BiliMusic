    ...
    fun parseSearchResult(json: String): List<NeteaseSong> {
        ...
        for (i in 0 until songs.length()) {
            ...
            list.add(NeteaseSong(
                ...
                genre = s.optString("genre", ""),
                language = s.optString("language", ""),
                pop = s.optDouble("pop", 0.0).toFloat()
            ))
        }
        return list
    }

    fun parseDailyRecommend(json: String): List<NeteaseSong> {
        val root = org.json.JSONObject(json)
        if (root.optInt("code") != 200) return emptyList()
        val data = root.optJSONObject("data") ?: return emptyList()
        val songs = data.optJSONArray("dailySongs") ?: return emptyList()
        val list = mutableListOf<NeteaseSong>()
        for (i in 0 until songs.length()) {
            val s = songs.getJSONObject(i)
            val artists = mutableListOf<NeteaseArtist>()
            val arr = s.optJSONArray("ar") ?: s.optJSONArray("artists")
            if (arr != null) {
                for (j in 0 until arr.length()) {
                    val a = arr.getJSONObject(j)
                    artists.add(NeteaseArtist(a.optLong("id"), a.optString("name")))
            ...
            // Try multiple cover URL sources
            val coverUrl = albumObj?.picUrl
                ?: s.optString("coverUrl").takeIf { it.isNotBlank() }
                ?: s.optString("cover").takeIf { it.isNotBlank() }

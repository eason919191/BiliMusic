...
import com.bili.music.MainActivity
...
class MusicPlayer @Inject constructor(
    ...
    init {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            NotificationChannel("playback", context.getString(com.bili.music.R.string.notif_channel_playback_name), NotificationManager.IMPORTANCE_LOW).apply {
                description = context.getString(com.bili.music.R.string.notif_channel_playback_desc)
                setShowBadge(false)
    ...
    private fun updateNotification(song: Music?, playing: Boolean) {
        ...
        val notification = NotificationCompat.Builder(context, "playback")
            ...
            .addAction(android.R.drawable.ic_media_previous, context.getString(com.bili.music.R.string.player_previous), null)
            .addAction(if (playing) android.R.drawable.ic_media_pause else android.R.drawable.ic_media_play,
                if (playing) context.getString(com.bili.music.R.string.player_pause) else context.getString(com.bili.music.R.string.player_play), null)
            .addAction(android.R.drawable.ic_media_next, context.getString(com.bili.music.R.string.player_next), null)
            .apply {
        ...
        com.bili.music.service.MusicService.setNotification(notification)
    ...
    fun playSong(song: Music, songList: List<Music>? = null) {
        try {
            ...
            if (!isServiceStarted) {
                try {
                    context.startForegroundService(Intent(context, com.bili.music.service.MusicService::class.java))
            ...
            if (url.startsWith("/") || url.startsWith("file://")) {
                mp.setDataSource(url.removePrefix("file://"))
            } else {
    ...
    fun playNext() {
        ...
        if ((nextSong.url == null && nextSong.localPath == null) && nextSong.bvid != null) {
            scope.launch {
                try {
                    val detail = com.bili.music.data.api.BilibiliApiClient.getVideoDetail(nextSong.bvid!!)
                    if (detail != null) {
                        val url = com.bili.music.data.api.BilibiliApiClient.getAudioUrl(nextSong.bvid!!, detail.cid)
                        if (url != null) {
    ...
    fun seekTo(pos: Long) {
        try { mediaPlayer?.seekTo(pos.toInt()); _currentPosition.value = pos } catch (_: Exception) {}
    }
    ...
    fun stop() {
        ...
        if (isServiceStarted) {
            try {
                context.stopService(Intent(context, com.bili.music.service.MusicService::class.java))
            } catch (_: Exception) {}

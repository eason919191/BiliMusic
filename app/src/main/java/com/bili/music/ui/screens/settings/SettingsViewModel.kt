...
import com.bili.music.data.api.BilibiliLoginClient
import com.bili.music.data.api.QRCodeData
import com.bili.music.data.api.UserInfo
...
class SettingsViewModel @Inject constructor(
    ...
    // ===== Account =====
    /**
     * Cookie登录：需要完整的Cookie字符串（含SESSDATA和DedeUserID）
     */
        ...
        viewModelScope.launch {
            try {
                ...
                if (qrData != null) {
                    ...
                    _uiState.update {
                        it.copy(
                            qrCodeMessage = "获取二维码失败（B站接口可能返回了空数据）",
    ...
    private suspend fun pollQRCodeResult(qrcodeKey: String) {
        ...
        while (retryCount < 120) { // 最多等120秒
            ...
            if (success) {
                ...
                _uiState.update { it.copy(qrCodeMessage = "登录成功！", isQrPolling = false) }
                return
            }
            when {
                message.contains("过期") -> {
    ...
    fun loadFavoriteFolders() {
        ...
        viewModelScope.launch {
            try {
                ...
                if (folders.isEmpty()) {
                    _uiState.update {
                        it.copy(isLoadingFolders = false, error = "未找到收藏夹（请确认B站账号有收藏的视频）")
                    }
                } else {

package com.bili.music.ui.screens.playlist
        ...
        viewModelScope.launch {
            try {
                ...
                if (song.bvid != null && song.bvid.isNotBlank()) {
                    val detail = com.bili.music.data.api.BilibiliApiClient.getVideoDetail(song.bvid)
                    val cid = detail?.cid ?: 0L
                    if (cid > 0) {
                        // 下载专用：获取DASH纯音频流（文件小，3-8MB）
                        val dashUrl = com.bili.music.data.api.BilibiliApiClient.getDownloadAudioUrl(song.bvid, cid)
